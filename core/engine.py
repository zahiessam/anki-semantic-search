...
def get_embedding_via_local_openai(text, base_url="http://localhost:1234/v1", model=None):
    """Fetch a single embedding from a local OpenAI-compatible server (LM Studio, Jan, etc.)."""
    ...
    try:
        ...
        raise RuntimeError(f"Unexpected local embeddings response: {type(resp_data)}")
    except Exception as e:
        log_debug(f"Local OpenAI embedding failed: {str(e)}")
        raise
...
def get_embedding_for_query(text, config=None):
    ...
    if engine == "local_openai":
        url = (sc.get("local_llm_url") or "http://localhost:1234/v1").strip()
        model = (sc.get("local_llm_model") or "model-identifier").strip()
        return get_embedding_via_local_openai(text, base_url=url, model=model)
...
def get_embeddings_via_local_openai_batch(texts, base_url="http://localhost:1234/v1", model=None):
    """Fetch multiple embeddings from a local OpenAI-compatible server."""
    if not texts:
        return []
    # Local servers often have limits or are slower, but many support batching.
    # We'll use a simple loop or direct batch if supported.
    url = base_url.rstrip("/") + "/embeddings"
    ...
    try:
        ...
        if isinstance(resp_data, dict) and resp_data.get("data"):
            ...
            return embeddings
        raise RuntimeError(f"Unexpected local batch embeddings response: {type(resp_data)}")
    except Exception as e:
        log_debug(f"Local OpenAI batch embedding failed: {str(e)}")
        # Fallback to individual requests if batch fails
        out = []
        for t in texts:
            out.append(get_embedding_via_local_openai(t, base_url=base_url, model=model))
        return out
...
def get_embeddings_batch(texts, input_type="document", config=None):
    ...
    if engine == "local_openai":
        url = (sc.get("local_llm_url") or "http://localhost:1234/v1").strip()
        model = (sc.get("local_llm_model") or "model-identifier").strip()
        return get_embeddings_via_local_openai_batch(texts, base_url=url, model=model)
