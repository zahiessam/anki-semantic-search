...
class QueryRefinementDialog(QDialog):
    """Adaptive question dialog to help users refine their medical queries."""

    def __init__(self, initial_query, parent=None):
        super().__init__(parent)
        self.setWindowTitle("✨ Refine Your Question")
        self.setMinimumWidth(500)
        self.refined_query = initial_query
        ...
        self.setStyleSheet(f"""
            ...
            QGroupBox {{ border: 1px solid {theme['border']}; border-radius: 6px; margin-top: 10px; padding: 10px; color: {theme['accent']}; font-weight: bold; }}
            QPushButton {{ padding: 8px 16px; border-radius: 6px; font-weight: bold; }}
            #refineBtn {{ background-color: {theme['accent']}; color: white; border: none; }}
            #cancelBtn {{ background-color: {theme['muted_btn']}; color: white; border: none; }}
        """)

        layout = QVBoxLayout(self)

        header = QLabel(f"<b>Current query:</b> \"{initial_query}\"")
        header.setWordWrap(True)
        layout.addWidget(header)

        # Clinical Focus Group
        self.focus_group = QGroupBox("What is your clinical focus?")
        focus_layout = QVBoxLayout()

        self.options = [
            ("General Overview", "Explain {query} in detail."),
            ("Pathophysiology", "What is the underlying mechanism and pathophysiology of {query}?"),
            ("Clinical Presentation", "What are the classic signs, symptoms, and physical exam findings for {query}?"),
            ("Diagnostics/Workup", "What is the gold standard diagnostic test and initial workup for {query}?"),
            ("Management/Pharmacology", "What is the first-line treatment, mechanism of action of drugs, and management algorithm for {query}?"),
            ("Differential Diagnosis", "What are the key differentials and how do you distinguish {query} from them?"),
            ("Compare & Contrast", "Compare and contrast {query} with its closest clinical mimics. Focus on distinguishing features, unique labs, and management differences.")
        ]

        self.radio_buttons = []
        for i, (label, template) in enumerate(self.options):
            rb = QRadioButton(label)
            if i == 0: rb.setChecked(True)
            focus_layout.addWidget(rb)
            self.radio_buttons.append((rb, template))

        self.focus_group.setLayout(focus_layout)
        layout.addWidget(self.focus_group)

        # Expert Controls
        expert_layout = QHBoxLayout()
        self.high_yield_check = QCheckBox("High-Yield (Bullet Points)")
        self.high_yield_check.setChecked(True)
        self.high_yield_check.setToolTip("Forces the AI to respond in concise, exam-style bullet points.")

        self.level_combo = QComboBox()
        self.level_combo.addItems(["Med Student Level", "Resident/Physician Level", "Explain Like I'm 5"])
        self.level_combo.setToolTip("Adjusts the complexity of the AI explanation.")

        expert_layout.addWidget(self.high_yield_check)
        expert_layout.addWidget(QLabel(" Complexity:"))
        expert_layout.addWidget(self.level_combo)
        layout.addLayout(expert_layout)

        # Additional Context
        layout.addWidget(QLabel("<b>Additional specific details (optional):</b>"))
        self.detail_input = QLineEdit()
        self.detail_input.setPlaceholderText("e.g., in elderly patients, side effects, etc.")
        ...
        refine_btn = QPushButton("Use Refined Query")
        refine_btn.setObjectName("refineBtn")
        refine_btn.clicked.connect(self.accept_refinement)

        btn_layout.addStretch()
        btn_layout.addWidget(cancel_btn)
        btn_layout.addWidget(refine_btn)
        layout.addLayout(btn_layout)

    def accept_refinement(self):
        query = self.refined_query
        template = ""
        for rb, t in self.radio_buttons:
            if rb.isChecked():
                template = t
                break

        refined = template.format(query=query)
        details = self.detail_input.text().strip()
        if details:
            refined += f" Focus specifically on: {details}"

        self.refined_query = refined
        self.accept()

def _addon_theme(is_dark):
...
def count_notes_matching_config(ntf):
    ...
    try:
        ...
        for m in mw.col.models.all():
            mid = m['id']
            m_name = m['name']
            if enabled_set and m_name not in enabled_set:
                continue
            flds = m['flds']
            if search_all:
...
from .ui.search_dialog import RelevanceBarDelegate, ContentDelegate
    ...
    def run(self):
        ...
        try:
            ...
            if self.checkpoint and self.resume_available:
                ...
                self.log_message.emit(f"\U0001F4CB Starting from previous checkpoint: {len(processed_note_ids):,} notes (latest checkpoint will update below as we save)")
            ...
            def maybe_save_checkpoint():
                ...
                self.log_message.emit(f"\U0001F4CB Checkpoint saved: {next_checkpoint:,} / {self.note_count:,} notes")
            ...
            def process_pending_batch():
                ...
                try:
                    ...
                    if errors <= 10:
                        ...
                        self.log_message.emit(f"\xe2\u0161\xa0\ufe0f Batch embedding error: {str(batch_err)[:50]} - falling back to per-note")
                    ...
                    for nid, content_hash, note_content in pending_notes:
                        ...
                        try:
                            ...
                            if errors <= 10:
                                ...
                                self.log_message.emit(f"\xe2\u0161\xa0\ufe0f Error generating embedding for note {nid}: {str(embed_error)[:50]}")
                ...
                if not embeddings_list or len(embeddings_list) != len(pending_notes):
                    ...
                    if errors <= 10:
                        ...
                        self.log_message.emit("\xe2\u0161\xa0\ufe0f Batch embedding error: mismatched embeddings count")
            ...
            for nid, mid, flds_str in mw.col.db.execute(f"select id, mid, flds from notes where id in ({id_list})"):
                ...
                if self.isInterruptionRequested():
                    self.log_message.emit("\xe2\u0161\xa0\ufe0f Thread interruption requested. Stopping...")
                    break

                # Check for pause (with interruption check)
                while self._is_paused:
                    if self.isInterruptionRequested():
                        self.log_message.emit("\xe2\u0161\xa0\ufe0f Thread interruption requested during pause. Stopping...")
                        break
                    time.sleep(0.1)
                ...
                try:
                    ...
                    # Check if embedding already exists for this engine
                    ...
                    errors += 1
                    if errors <= 10:
                        self.log_message.emit(f"\xe2\u0161\xa0\ufe0f Error processing note {nid}: {str(e)[:100]}")
                    continue
            ...
            if not self.isInterruptionRequested():
                ...
                self.log_message.emit(f"\U0001F4CB Final checkpoint saved: {processed:,} / {self.note_count:,} notes")
            ...
            if failed_notes and not self.isInterruptionRequested():
                ...
                self.log_message.emit(f"\U0001F504 Retrying {len(failed_notes)} failed note(s)...")
                ...
                for idx, (nid, (content_hash, note_content)) in enumerate(retry_list):
                    ...
                    try:
                        ...
                        if nid in failed_notes and errors <= 15:
                            ...
                            self.log_message.emit(f"\xe2\u0161\xa0\ufe0f Retry failed for note {nid}: {str(retry_err)[:50]}")

                flush_embedding_batch()

                if failed_notes:

                    self.log_message.emit(f"\xe2\u0161\xa0\ufe0f {format_partial_failure_progress(len(failed_notes))}")
    ...
    def _test_local_server_connection(self):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                showInfo(f"⚠️ Server responded with code {resp.status_code}.\nURL: {test_url}")
        except Exception as e:
...
'''
    ...
    def _on_embedding_finished(self, progress_dialog, processed, errors, skipped, still_failed_count, note_count):
        ...
        if still_failed_count > 0:
            ...
            log_text.append(f"\xe2\u0161\xa0\ufe0f {format_partial_failure_progress(still_failed_count)}")
        ...
        if still_failed_count > 0:
            ...
            message += f"\n\n\xe2\u0161\xa0\ufe0f {format_partial_failure_completion(still_failed_count)}"
...
class QueryRefinementDialog(QDialog):
    """Adaptive UI to refine and polish medical queries using AI."""

    def __init__(self, initial_query, parent=None):
        super().__init__(parent)
        self.setWindowTitle("\U0001FA84 Clinical Query Refiner")
        ...
        self.setStyleSheet(f"""
            ...
            QGroupBox {{ border: 1px solid {theme['border']}; border-radius: 6px; margin-top: 10px; padding: 10px; color: {theme['accent']}; font-weight: bold; }}
            QPushButton {{ padding: 8px 16px; border-radius: 6px; font-weight: bold; }}
            QLineEdit {{ background-color: {theme['input_bg']}; color: {theme['input_text']}; border: 1px solid {theme['border']}; padding: 6px; border-radius: 4px; }}
            QTextEdit {{ background-color: {theme['input_bg']}; color: {theme['input_text']}; border: 1px solid {theme['border']}; }}
        """)

        # Header
        header = QLabel("AI Clinical Refinement")
        ...
        self.original_text.setText(initial_query)
        self.original_text.setStyleSheet(f"background-color: {theme['panel_bg']}; color: {theme['subtext']};")
        layout.addWidget(self.original_text)

        # Step 1: Clinical Focus (Multiple Selection)
        self.focus_group = QGroupBox("1. What is your clinical focus? (Select multiple)")
        focus_layout = QVBoxLayout()
        self.options = [
            ("General Overview", "comprehensive overview"),
            ("Pathophysiology & Mechanism", "underlying mechanism and pathophysiology"),
            ("Clinical Presentation (S/Sx)", "classic signs, symptoms, and physical exam findings"),
            ("Diagnostics & Workup", "gold standard diagnostic tests and initial workup"),
            ("Management & Pharmacology", "first-line treatment, drug mechanisms, and management algorithms"),
            ("High-Yield Differentials", "key differential diagnoses and distinguishing features")
        ]
        self.check_boxes = []
        for i, (label, focus_term) in enumerate(self.options):
            cb = QCheckBox(label)
            if i == 0: cb.setChecked(True)
            cb.stateChanged.connect(self._re_run_ai_with_context)
            focus_layout.addWidget(cb)
            self.check_boxes.append((cb, focus_term))
        self.focus_group.setLayout(focus_layout)
        layout.addWidget(self.focus_group)

        # Step 2: AI Result
        layout.addWidget(QLabel("<b>2. Polished Clinical Query:</b>"))
        self.refined_text = QTextEdit()
        self.refined_text.setPlaceholderText("Asking AI to polish your query...")
        self.refined_text.setStyleSheet(f"font-size: 14px; border: 2px solid {theme['accent']};")
        layout.addWidget(self.refined_text)

        # Additional Context
        layout.addWidget(QLabel("<b>3. Add specific details (optional):</b>"))
        self.detail_input = QLineEdit()
        self.detail_input.setPlaceholderText("e.g., focus on elderly, side effects, etc.")
        self.detail_input.textChanged.connect(self._on_details_changed)
        layout.addWidget(self.detail_input)

        # Action Buttons
        btn_layout = QHBoxLayout()
        self.apply_btn = QPushButton("\u2705 Apply Refinement")
        self.apply_btn.setStyleSheet(f"background-color: {theme['success']}; padding: 10px; color: white;")
        self.apply_btn.clicked.connect(self.accept)

        self.cancel_btn = QPushButton("\u2716 Discard")
        self.cancel_btn.setStyleSheet(f"background-color: {theme['muted_btn']}; padding: 10px; color: white;")
        ...
        # Automatically start refinement
        self._timer = QTimer(self)
        self._timer.setSingleShot(True)
        self._timer.timeout.connect(self._start_ai_refinement)
        self._timer.start(100)

    def _on_details_changed(self):
        # Debounce the AI call so it doesn't fire on every keystroke
        self._timer.stop()
        self._timer.start(1000)

    def _re_run_ai_with_context(self):
        self._start_ai_refinement()

    def _start_ai_refinement(self):
        query = self.original_text.text().strip()
        if not query: return

        # Combine selected focuses
        selected_focuses = [term for cb, term in self.check_boxes if cb.isChecked()]
        if not selected_focuses:
            focus_str = "general clinical overview"
        else:
            focus_str = ", ".join(selected_focuses)

        details = self.detail_input.text().strip()
        contextual_query = f"Refine the medical topic '{query}' focusing on {focus_str}."
        if details:
            contextual_query += f" Also consider: {details}"

        self.refined_text.setPlaceholderText("Glutamine is synthesizing your request...")
        if hasattr(self.parent(), "run_refinement_ai"):
            self.parent().run_refinement_ai(contextual_query, self.refined_text)

    def get_refined_query(self):
        return self.refined_text.toPlainText().strip()
...
class AISearchDialog(QDialog):
    ...
    def run_refinement_ai(self, query, output_widget):
        """Asynchronously call the LLM to refine the medical query with a final sanity check."""
        from .llm import get_provider, create_payload

        config = mw.addonManager.getConfig(__name__)
        provider_name = config.get("provider", "OpenAI")
        api_key = config.get("api_key", "")

        # The 'Glutamine' Medical Refinement Prompt with Checkpoint
        system_prompt = """
        You are an expert medical editor and clinical educator (Glutamine).
        Your goal is to refine messy student queries into high-signal semantic search strings for medical Anki cards.

        CRITICAL RULES:
        1. NEVER return the original query unchanged.
        2. ENRICH with technical synonyms (e.g., "EKG" -> "Electrocardiogram, ST-elevation, arrhythmias").
        3. STRIP filler ("tell me about", "what is").
        4. OUTPUT ONLY the refined string. No filler.

        FINAL CHECKPOINT RULE:
        Before outputting, perform a 'Clinical Sanity Check':
        - Remove any repeated words (e.g., no "shock shock").
        - Ensure medical terms are spelled correctly.
        - If the result looks like nonsense or is just the original word repeated, expand it further into its clinical components.
        """

        user_prompt = f"Refine this medical query: {query}"

        # Show 'Thinking' state
        output_widget.setPlaceholderText("Glutamine is performing a final sanity check...")

        def on_done(refined_text):
            # Check if the widget still exists before trying to access it
            try:
                # sip.isdeleted or simply checking if the underlying C++ object is null
                if not output_widget or not hasattr(output_widget, 'setPlainText'):
                    return
            except RuntimeError:
                return

            if refined_text:
                # Python-side Final Checkpoint: Deduplicate and Clean
                text = refined_text.strip()

                # 1. Remove double spaces and nonsense punctuation
                import re
                text = re.sub(r'\s+', ' ', text)
                text = re.sub(r'([!?.,])\1+', r'\1', text)

                # 2. Case-insensitive word deduplication (keeps medical context)
                words = text.split()
                seen = []
                for w in words:
                    if w.lower() not in seen:
                        seen.append(w)
                clean_text = " ".join(seen)

                output_widget.setPlainText(clean_text)
            else:
                output_widget.setPlainText(query)
        ...
        class RefinementWorker(QThread):
            ...
            def run(self):
                try:
                    ...
                    # Note: Using a low temperature for precision
                    payload = create_payload(provider_name, system_prompt, user_prompt, temperature=0.2)
                    response = provider.generate(payload)
                    self.finished.emit(response)
                except Exception as e:
                    log_debug(f"Refinement error: {str(e)}")
                    self.finished.emit("")

        self._refine_worker = RefinementWorker()
        self._refine_worker.finished.connect(on_done)
        self._refine_worker.start()

    def on_refine_clicked(self):
        """Show the adaptive query refinement dialog."""
        current_text = self.search_input.toPlainText().strip()
        if not current_text:
            tooltip("Enter a basic topic first to refine it.")
            return

        dialog = QueryRefinementDialog(current_text, self)
        if dialog.exec():
            refined = dialog.get_refined_query()
            if refined:
                self.search_input.setPlainText(refined)
    ...
    def setup_ui(self):
        ...
        self.refine_btn = QPushButton("✨")
        self.refine_btn.setToolTip("Refine question for better AI answer")
        self.refine_btn.setMinimumHeight(42)
        self.refine_btn.setMaximumWidth(40)
        self.refine_btn.setStyleSheet(f"background-color: {theme['accent']}; border: none; font-size: 16px;")
        ...
        search_input_layout.addWidget(self.refine_btn)
...
# but are copied to AISearchDialog at module load (see _aisearch_methods_from_worker).
    ...
    def run_refinement_ai(self, query, output_widget):
        """Asynchronously call the AI to refine the medical query."""
        from .llm import get_llm_provider

        config = mw.addonManager.getConfig(__name__)
        provider_name = config.get("provider", "ollama")

        # Clinical Refinement Prompt (Scrubbed)
        system_prompt = """
        You are an expert medical editor and clinical educator.
        Your goal is to refine messy student queries into high-signal semantic search strings.

        RULES:
        1. STRIP redundant phrases: "What is", "Tell me about", "Can you find", "search for".
        2. FIX grammar and REMOVE repeated words.
        3. ENRICH with medical synonyms: If they say "Heart Failure", add "(CHF, HFrEF, HFpEF)".
        4. STRUCTURE comparisons: "A vs B" should become a structured comparative query.
        5. OUTPUT ONLY the refined search string. No conversational filler.

        Goal: Maximize 'Cosine Similarity' for a vector database of Anki cards.
        """

        user_prompt = f"Refine this medical query: {query}"

        # Show 'Thinking' state
        output_widget.setPlaceholderText("AI is refining your query...")
        ...
        def on_done(refined_text):
            ...
            if refined_text:
                output_widget.setPlainText(refined_text.strip())
            else:
                output_widget.setPlainText(query) # Fallback to original

        class RefinementWorker(QThread):
            finished = Signal(str)
            def __init__(self, query_text, p_name, cfg):
                super().__init__()
            ...
            def run(self):
                try:
                    provider = get_llm_provider(self.p_name, self.cfg)
                    full_prompt = f"{system_prompt}\n\nUser: {self.query_text}\nRefined:"
                    response = provider.generate_answer(full_prompt)
                    self.finished.emit(response)
                except Exception as e:
                    log_debug(f"Refinement error: {str(e)}")
                    self.finished.emit("")

        self._refine_worker = RefinementWorker(query, provider_name, config)
        self._refine_worker.finished.connect(on_done)
        self._refine_worker.start()
...
class SidebarButton(QWidget):
    def __init__(self, icon, text, theme, callback, parent=None):
        ...
        # Main layout for the whole button area (Icon + Sliding Text)
        self.layout = QHBoxLayout(self)
        self.layout.setContentsMargins(0, 0, 0, 0)
        self.layout.setSpacing(0)

        # 1. The Icon Pill (Left Side - Fixed)
        self.icon_frame = QFrame()
        self.icon_frame.setFixedSize(40, 40)
        self.icon_frame.setStyleSheet(f"background: transparent; border: none;")
        ...
        # 2. The Text Drawer (Right Side - Sliding)
        self.drawer = QFrame()
        self.drawer.setObjectName("drawer")
        self.drawer.setMaximumWidth(0) # Starts closed
        ...
        self.layout.addStretch() # Ensure alignment stays left
    ...
    def mousePressEvent(self, event):
        # We handle the action in release for better UX
        super().mousePressEvent(event)

class SemanticSearchSideBar(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        # Remove WindowStaysOnTopHint so it doesn't override other dialogs
        self.setWindowFlags(Qt.WindowType.FramelessWindowHint | Qt.WindowType.Tool)
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)

        # The root layout is a vertical stack of buttons
        ...
        # Create the icon container pill (The background for just the icons)
        self.icon_pill = QFrame(self)
        self.icon_pill.setFixedWidth(44)
        self.icon_pill.setFixedHeight(144)
        self.icon_pill.setStyleSheet(f"""
            QFrame {{
                background-color: {theme['bg']};
                border: 1px solid {theme['panel_border']};
                border-radius: 22px;
        ...
        # Position buttons inside the pill, but let them expand to the right
        # Offset to center icons in the 44px pill (44px pill - 40px icon_frame = 2px margin)
        self.root_layout.setContentsMargins(2, 4, 0, 0)

        self.btn_search = SidebarButton("\U0001F50D", "Search", theme, show_search_dialog, self)
        self.btn_embed = SidebarButton("\U0001F4DA", "Embed", theme, lambda: show_settings_dialog(open_to_embeddings=True, auto_start_indexing=True), self)
        self.btn_settings = SidebarButton("\u2699", "Settings", theme, lambda: show_settings_dialog(False), self)
        ...
        self.setFixedWidth(160) # Total area allowed for drawers
        self.setFixedHeight(160)
        self.setMouseTracking(True)

    def paintEvent(self, event):
        # We need to manually draw the icon pill background because
        # the buttons actually extend past it when expanded.
        painter = QPainter(self)
        # Fix for Qt6: Antialiasing is in RenderHint enum
        ...
        # Draw the vertical pill on the left
        rect = QRect(0, 0, 44, 144)
        painter.drawRoundedRect(rect, 22, 22)

    # Allow the window to resize to fit the expanding text
...
def update_sidebar_position():
    if _sidebar_instance and mw and mw.isVisible():
        # Don't show or move if search is active (prevents re-opening bug)
        if _ai_search_dialog_instance and _ai_search_dialog_instance.isVisible():
            _sidebar_instance.hide()
            return

        geom = mw.geometry()
        # Bottom-left corner, slightly offset
        padding = 20
        x = geom.x() + padding
        y = geom.y() + geom.height() - _sidebar_instance.height() - padding - 40 # 40 for bottom bar
...
def show_sidebar():
    global _sidebar_instance
    if not _sidebar_instance:
        _sidebar_instance = SemanticSearchSideBar(mw)
    update_sidebar_position()
...
def _on_main_window_did_init():
    ...
    # Refresh position on state changes (e.g., entering Reviewer)
    from aqt import gui_hooks
    gui_hooks.state_did_change.append(lambda _new_state, _old_state: update_sidebar_position())

    # Event filter to track Anki window movement/resize
    if hasattr(mw, "installEventFilter"):
        class MoveFilter(QObject):
            def eventFilter(self, obj, event):
                if event.type() in (QEvent.Type.Resize, QEvent.Type.Move, QEvent.Type.Show):
                    update_sidebar_position()
                return False
        mw._move_filter = MoveFilter(mw)
        mw.installEventFilter(mw._move_filter)

    # Add sidebar to main window

    try:
        try:
            from aqt.sidebar import SidebarItem, SidebarFolder
        except ImportError:
            # Fallback for even newer or different Anki structures
            from aqt.browser.sidebar import SidebarItem, SidebarFolder

        from aqt import gui_hooks



        def on_sidebar_will_filter(tree):

            root = tree.root

            # Find or create a folder for this addon

            folder = tree.find_item("\U0001F50D Semantic Search")

            if not folder:

                folder = SidebarItem(

                    name="\U0001F50D Semantic Search",

                    id="semantic_search_root",
                ...
                tree.insert_item(folder, root)



            # Add Search item

            search_item = SidebarItem(
            ...
            tree.insert_item(search_item, folder)
            ...
            settings_item = SidebarItem(
                ...
                id="semantic_search_settings",

                on_click=lambda: show_settings_dialog(False),

                icon="\u2699"

            )

            tree.insert_item(settings_item, folder)



        # For Anki 23.10+

        if hasattr(gui_hooks, "sidebar_will_filter"):

            gui_hooks.sidebar_will_filter.append(on_sidebar_will_filter)

    except Exception as e:

        log_debug(f"Could not add sidebar items: {e}")
...
# Clear the log on startup to keep it fresh and readable
try:
    clear_log()
    log_debug("Addon initialized and log cleared for fresh session.")
except Exception:
    pass
